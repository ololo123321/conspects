...
Утверждается, что результаты в точности совпадают с libpq, хотя это не очевидно: pqdistance считается лишь для малой доли векторов, которая отбирается по оценке нижней границы расстояния до запроса. Но это нижняя граница имеет ошибку квантизации $D_j \rightarrow S_j$. Следовательно, могут иметь место пропуски ближайших соседей.
