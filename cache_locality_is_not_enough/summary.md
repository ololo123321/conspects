...
Пусть $f(x) = bin(x)$ - функция квантизации, используемая для получения $S_j$. Далее, пусть $lb(p)$ - функция получения точной $lower\_bound$, $lb_q(p)$ - функция получения $lower\_bound$, квантизированной с помощью $f$ (п. 2.3.1. алгоритма). Так как $f(x)$ - монотонно возрастает по $x$, то $lb_q(p_0) < lb_q(p_1) \Rightarrow lb(p_0) < lb(p_1)$.
