...
Пусть $f(x) = bin(x)$ - функция квантизации, используемая для получения $S_j$. Далее, пусть $lb_q(p)$ - функция получения $lower\_bound$ из п.2.3.1., $lb(p)$ - та же функция, только $S_j -> D_j$. Вообще говоря, неверно, что $lb_q(p_0) < lb_q(p_1) \Rightarrow lb(p_0) < lb(p_1)$.

Это можно показать на примере: пусть $p_0$ таков, что $S_j[p_0[j][0]] = 0, 4 \le j < 8$, хотя $D_j[p_0[j]] = b - \epsilon$, где $b$ - верхняя граница нулевого бина, а $\epsilon$ - небольшое положительное число. Таким образом, $lb_q(p_0) = 0; lb(p_0) = 4(b - \epsilon)$.

Далее, пусть $p_1$ таков, что $S_j[p_1[j][0]] = 0, D_j[p_1[j]] = q_\text{min}$, если $4 \le l < 7$, и $S_7[p_1[j][0]] = 1, D_7[p_1[7]] = b + \epsilon$.
В таком случае $lb_q(p_1) = 1; lb(p_1) = 3q_\text{min} + b + \epsilon$. 

Несмотря на то, что $lb_q(p_0) < lb_q(p_1)$, неравенство $lb(p_0) > lb(p_1)$
выполняется при $b > q_\text{min} + \frac{5}{3}\epsilon$. 
