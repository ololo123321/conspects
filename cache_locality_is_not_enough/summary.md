...
* $S_j$ используются для подсчёта нижних границ расстояний между запросом $q$ и хранимым вектором  $p$. Идея в том, что такая штука позволяет сильно снизить количество расчётов точных расстояний (в статье говортся, что примерно на 95%), для расчёта которых требуется лезть в L1-кэш. Зачение от меня: если требуется искать искать $p$, максимизирующий $q \cdot p$, то задача разворачивается в сторону максимизации.
